...
sub parse_citation {
	my ($self, $citation) = @_;
	debug("parse_citation() in GODOT::Parser::atlaf") if $GODOT::Config::TRACE_CALLS;
	...
	my $source = $citation->parsed('SOURCE');
	    ...
	    if ($source =~ /^.*\.\s+[^0-9]*(\d+)[\s,]*([Nn]o){0,1}\s*([0-9-]*)\s+\((.*)\s*\d{4,4}\),\s+p\.\s+([0-9,+-]*)/) {
	       ...
	       print STDERR "Warning: atlaf: parse_citation: journal parsing failed!\n";
	    }
	}
	## Examples:
	...
	# 'In: Zeit und Stunde. Munich : Maander Verlag, 1985. p. 235-255.'
	    ...
	    if ($source =~ /^In:\s+(.+)\..+:\s+(.+),\s+(\d{4,4})\.\s+p\.\s+([0-9+-]+)\./) {
	       ...
	       print STDERR "Warning: atlaf: parse_citation: book_article parsing failed!\n";
	    }
	}
...
sub get_req_type {
        my ($self, $citation) = @_;
        debug("get_req_type() in GODOT::Parser::atlaf") if $GODOT::Config::TRACE_CALLS;
	...
	##---------------Customized code goes here-------------------##
	    ...
	    print STDERR "Warning: atlaf: get_req_type: new req_type!"; 
	}
