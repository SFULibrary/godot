...
sub parse_citation {
	my ($self, $citation) = @_;
	debug("parse_citation() in GODOT::Parser::csa") if $GODOT::Config::TRACE_CALLS;
...
sub get_req_type {
        my ($self, $citation) = @_;
        debug("get_req_type() in GODOT::Parser::csa") if $GODOT::Config::TRACE_CALLS;
