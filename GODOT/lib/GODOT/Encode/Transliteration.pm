## GODOT::Encode::Transliteration
##
## Copyright (c) 2010, Kristina Long, Simon Fraser University
##
## Transliteration map for utf8 to latin1.
##
package GODOT::Encode::Transliteration;
...
sub utf8_to_latin1_transliteration_map {
    ...
    foreach my $line (split(/\n/, $string)) {
        ...
        ##
        ## -only take the first mapping
        ##
        my $to_string = _Utag_to_chr($o[0]);      

        ##
        ## -skip any line that does not transliterate to latin1;
        ##
        unless (is_latin1($to_string)) {
            #### debug "does not transliterate to latin1:  ", dump($to_string);
            next;
        }

        $map{ _Utag_to_chr($from) } = $to_string;
    }
...
## 
## (29-jan-2010 kl) -- Changed so that no characters in ascii range appear on left-hand side.  No need to transliterate these. 
##                  -- The two lines removed were: '<U0027> <U2019>' and '<U0060> <U201B>;<U2018>'
## 
sub utf8_transliteration_map_data {
