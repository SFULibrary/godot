package GODOT::CatalogueHoldings::Object;
...
sub AUTOLOAD {
    my $self = shift;
    my $class = ref($self) || error("CatalogueHoldings::Object::AUTOLOAD - self is not an object ($AUTOLOAD)");
    my $field = $AUTOLOAD;

    $field =~ s/.*://;               ## -strip fully qualified part

    unless (grep {$field eq $_} (@{$self->{'_permitted'}})) {
        error "CatalogueHoldings::AUTOLOAD - '$field' is an invalid field for an object of class $class";
    }
