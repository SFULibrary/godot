...
sub AUTOLOAD {
    my $self = shift;
    my $class = ref($self) || error("Template::Object::AUTOLOAD - self is not an object");
    my $field = $AUTOLOAD;

    $field =~ s/.*://;               ## -strip fully qualified part

    unless (grep {$field eq $_} (@{$self->{'_permitted'}})) {
        error "Template::AUTOLOAD - '$field' is an invalid field for an object of class $class";
    }
